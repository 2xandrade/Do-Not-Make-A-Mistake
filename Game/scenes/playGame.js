...
class PlayGame extends Phaser.Scene {
    ...
    setupTimers() {
        ...
        this.time.delayedCall(GameOptions.secondEnemy.spawnTime, () => {
            this.time.addEvent({
                delay: GameOptions.secondEnemy.spawnInterval, // Usa o intervalo configurável
                loop: true,
                callback: this.spawnSecondEnemy,
                callbackScope: this
            });
    ...
    setupCollisions() {
        // Colisão jogador vs todos inimigos
        this.physics.add.collider(this.player, this.enemyGroup, (player, enemy) => {
            this.takeDamage(enemy.damage || 1); // Usa damage do inimigo ou 1 como padrão
        });

        // Colisão bala vs inimigos (unificada para todos os tipos)
    ...
    // Método auxiliar para spawnar múltiplas moedas
